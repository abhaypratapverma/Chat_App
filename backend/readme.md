...
frontend	Client interface (React or similar). (Implementation details not included in this snapshot)	—
🎯 Purpose
...
Node.js + TypeScript — for scalable, type-safe microservices.

Express.js — for RESTful APIs.

MongoDB + Mongoose — for structured data storage.
...
✅ OTP-based Login Flow — Secure authentication using email OTPs stored in Redis.
✅ Microservice Communication via RabbitMQ — Decouples email sending from authentication flow.
✅ Real-time Chat Management — Create, manage, and fetch chats and messages seamlessly.
✅ Image Uploads — Integrated Cloudinary with Multer middleware.
✅ JWT Authentication — Tokens generated by user service, verified across services.
✅ Centralized Error Handling — TryCatch utility for consistent error management.
...
Microservice Separation — Ensures scalability and modularity.

Pub/Sub Email Delivery — Increases fault tolerance and performance.

Redis-backed OTP System — Fast, secure, and temporary data storage.

Mongoose Models — Data consistency and schema validation.

Unified Error Handling — Using custom TryCatch utility.
...
/api/v1/user/login	POST	Send OTP to user’s email.
...
(Refer to each service’s routes folder for complete details.)

🖼️ Example Flow: OTP Authentication

User enters email → POST /api/v1/user/login

OTP generated, stored in Redis → published to RabbitMQ queue

Mail service listens → sends OTP email

User enters OTP → POST /api/v1/user/verify-otp

JWT token generated and returned → used for all further requests
...
Abhay Pratap Verma — Lead Developer
...
- backend/user — Handles user authentication (OTP), user profile, and user management.
...
- backend/mail — Mail service that consumes RabbitMQ messages to send OTP emails.
  - Key files: [backend/mail/src/index.ts](backend/mail/src/index.ts), [`startSendOtpConsumer`](backend/mail/src/consumer.ts)
- backend/chat — Chat service that manages chats and messages, uploads images to Cloudinary.
...
- frontend — Client application (folder present but implementation details not included in workspace snapshot).
...
- Express (HTTP APIs) — used in each service
- MongoDB with Mongoose — data persistence (models in each service)
  - [`User`](backend/user/src/model/User.ts)
  - [`Chat`](backend/chat/src/models/Chat.ts)
  - [`Messages`](backend/chat/src/models/Messages.ts)
- Redis — temporary storage for OTPs and rate-limiting (used by user service) [backend/user/src/index.ts](backend/user/src/index.ts)
- RabbitMQ — messaging between user service and mail service ([backend/user/src/config/rabbitmq.ts](backend/user/src/config/rabbitmq.ts), [backend/mail/src/consumer.ts](backend/mail/src/consumer.ts))
- Cloudinary — image upload/storage for chat images ([backend/chat/src/config/cloudinary.ts](backend/chat/src/config/cloudinary.ts))
- Nodemailer — actually sends email from the mail service ([backend/mail/src/consumer.ts](backend/mail/src/consumer.ts))
...
- backend/
  - user/ — user service (OTP, user CRUD)
    - src/
  ...
  - chat/ — chat service (chats & messages)
    - src/
