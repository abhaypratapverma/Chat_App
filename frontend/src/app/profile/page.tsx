...
import { ArrowLeft, UserCircle, User, Save } from "lucide-react";
  ...
  const submitHandler = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${user_service}/api/v1/update/username`,
        { name },
        {
          headers: {
      ...
      setUser(data.user);
      setIsEdit(false); // ✅ corrected variable name (was setEdit)
    } catch (error: any) {
      console.error(error);
        ...
        <div className="bg-gray-800 rounded-lg border border-gray-700 shadow-lg">
          <div className="bg-gray-700 p-8 border-b border-gray-600">
            <div className="flex items-center gap-6">
              ...
              {/* ✅ Name and status now appear in the same row */}
